...
class HistoryPanel(QWidget):
    ...
    def __init__(self, storage, parent=None):
        ...
        # 显式设置新闻数据存储目录路径
        self.news_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "..", "..", "data", "news"
        )
    ...
    def _setup_import_tab(self, tab):
        ...
        # 添加说明文字
        instr_label = QLabel(
            "在此页面中，您可以导入外部JSON新闻文件或导出现有新闻数据。"
        )
        ...
        try:
            ...
            for filename in files:
                try:
                    ...
                    if filename.startswith("news_") and filename.endswith(".json"):
                        ...
                        display_text = (
                            f"{date_time.strftime('%Y-%m-%d %H:%M:%S')} ({filename})"
                        )
                    else:
            ...
            if self.export_combo.count() > 0:
                self.status_label.setText(
                    f"找到 {self.export_combo.count()} 个可导出文件"
                )
            else:
    ...
    def _load_to_main(self):
        ...
        try:
            ...
            self.logger.info(
                f"将历史文件 {filename} 中的 {len(news_items)} 条新闻加载到主界面"
            )

            # 显示成功消息
            QMessageBox.information(
                self, "加载成功", f"已成功加载 {len(news_items)} 条历史新闻到主界面"
            )
    ...
    def _export_selected_file(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "导出成功", f"成功导出 {len(news_items)} 条新闻到:\n{export_path}"
            )
