...
"""
    ...
    def _format_ai_response(self, text):
        ...
        if "<h3>" in text:
            text = text.replace(
                "<h3>",
                '<h3 style="font-size: 18px; margin-top: 12px; margin-bottom: 12px; color: #1976D2; font-weight: 600;">',
            )
        ...
        if "<p>" in text:
            text = text.replace(
                "<p>",
                '<p style="margin-top: 10px; margin-bottom: 10px; line-height: 1.8;">',
            )
        ...
        if "<ul>" in text:
            text = text.replace(
                "<ul>",
                '<ul style="margin-top: 10px; margin-bottom: 14px; padding-left: 25px;">',
            )

        if "<li>" in text:
            text = text.replace(
                "<li>", '<li style="margin-bottom: 8px; padding-left: 5px;">'
            )
        ...
        if "<code>" in text:
            text = text.replace(
                "<code>",
                '<code style="font-family: Consolas, Monaco, monospace; background-color: #F5F5F5; padding: 2px 4px; border-radius: 3px; border: 1px solid #E0E0E0;">',
            )

        # 强调元素格式化
        if "<strong>" in text:
            text = text.replace(
                "<strong>", '<strong style="font-weight: 600; color: #1565C0;">'
            )
        ...
        if "<blockquote>" in text:
            text = text.replace(
                "<blockquote>",
                '<blockquote style="border-left: 4px solid #BBDEFB; padding: 8px 12px; margin: 10px 0; background-color: #F5F5F5;">',
            )
